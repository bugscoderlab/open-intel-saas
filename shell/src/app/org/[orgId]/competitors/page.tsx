...
export default function CompetitorsModulePage() {
  return (
    <RequireAuth>
      <CompetitorsModule />
    </RequireAuth>
...
/** Org-level entry: pick a project (a market), then land in its
    ...
    <AppShell email={session.user.email ?? ""} orgs={orgs} currentOrgId={orgId}>
      <Card>
        <h2 className="mb-1 text-base font-semibold text-gray-900">
          Competitor-intelligence workspaces
        </h2>
        <p className="mb-4 text-sm text-gray-600">
          A project is a market: its competitor directory, service catalog,
          price observations, review queue, and evidence all live inside one.
          Choose one:
        </p>
        {error ? (
          ...
          <ul className="space-y-2">
            {projects.map((project) => (
              <li key={project.id}>
                <Link
                  href={`/org/${orgId}/projects/${project.id}?view=overview`}
