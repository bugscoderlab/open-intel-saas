...
def test_every_discovered_module_gets_a_state():
    states = build_module_states(ALL_MODULES, disabled=set())
    assert {s.name for s in states} == {"platform", "research", "analytics"}


def test_nothing_disabled_means_everything_enabled():
    states = build_module_states(ALL_MODULES, disabled=set())
    assert all(s.enabled for s in states)


def test_disabled_optional_module_is_reported_not_deleted():
...
def test_disabling_the_platform_module_is_rejected():
    with pytest.raises(ValueError, match="platform"):
        build_module_states(ALL_MODULES, disabled={"platform"})


def test_disabling_an_unknown_module_is_rejected():
    with pytest.raises(ValueError, match="typo"):
        build_module_states(ALL_MODULES, disabled={"typo"})


def test_platform_module_missing_is_rejected():
    with pytest.raises(ValueError, match="platform"):
